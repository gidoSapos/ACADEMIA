...
def cadastrar_exercicio(nome, musculos, carga, serie, repeticoes):
    ...
    try:
        with open(DATAPATH, "r+", encoding = 'utf-8') as arquivo:
    ...
    try:
        ...
        for exercicio in exercicios:
            if exercicio["nome"].lower() == nome.lower():
                ...
                musculos = simpledialog.askstring("Atualizar Exercício", "Músculos (separados por vírgula):", initialvalue =", ".join(exercicio["musculos"]))
                
                carga = simpledialog.askstring("Atualizar Exercício", "Carga (separados por vírgula):", initialvalue = ", ".join(exercicio.get("carga", [])))
...
def cadastrar_treino(nome, exercicios):
    ...
    try:
        with open(TREINOS_PATH, "r+", encoding = 'utf-8') as arquivo:
            dados = json.load(arquivo)
        ...
        with open(TREINOS_PATH, "w", encoding = 'utf-8') as arquivo:
